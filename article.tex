...
When applied to the previous example (Figure \ref{fig:countdown.ll}), only \texttt{\%argc} and \texttt{\%c2} may be coalesced with \texttt{\%c1} as this is the only mov instruction. 

Consider the above example but with some \(k \geq 2\). the \%c1 and 

\begin{figure}[H]
     \centering
   \begin{minipage}[b]{0.42\textwidth}
     \centering
     \includesvg[height=10em]{countdown.briggs}
     \caption{\texttt{dune exec build -{}- -t dot}}\label{fig:countdown.briggs.dot}
   \end{minipage}
\end{figure}

The purpose is only to assign the same registers to different variables. You don't need to note down information about blocks etc.
