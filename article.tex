...
If at first a single node of insignificant degree (i.e. \(< 2\)) is selected, it is then removed and pushed to a stack. In Figure \ref{fig:countdown.dot} only \texttt{\%argv} and \texttt{\%c1} are of insignificant degree, so either of those is chosen to be removed (represented with dashed borders and edges).
...
\begin{figure}[H]
  ...
  \end{minipage}%
\end{figure}
\noindent From this point onwards  nothing can be done to further simplify the graph as all of the remaining nodes are of significant degree (\(\leq 2\)). Therefore, one of the remaining nodes is spilled to the memory stack and removed completely from the graph.
...
\begin{figure}[H]
  ...
  \begin{minipage}[c]{0.32\linewidth}
    ...
    \begin{subfigure}[b]{0.27\textwidth}
      ...
      \caption{Stack}\label{fig:countdown.dot2}
    \end{subfigure}
    \caption{Image B}
  \end{minipage}%
\end{figure}
...
\centering
  \begin{minipage}[b]{0.48\textwidth}
    \begin{subfigure}{0.3\textwidth}
      \includesvg[height=12em]{countdown.simple2}
      \caption{Graph}\label{fig:countdown.dot2}
    \end{subfigure}
\hfill
