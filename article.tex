...
\section*{Abstract}
In this day and age compilers are used in more places than one would think of at a glance.
While programmers heavily rely on them to get their work done in a reasonable amount of time, %, as the ability to manually construct machine code is a ridiculous expectation to have.
 quite a significant amount of time that could otherwise have been spent being productive
 is spent waiting on the compiler to finish, hence the productivity in the field of software development stands to benefit from improved efficiency.
 Likewise, programming languages that used to be run by interpreting source text text chunks at a time are now being  translated to raw machine instructions immediately prior to execution just to squeeze out fractions of performance benefits on computers as Moore's law seems to nearing it's natural limit as dictated by the laws of our universe. 
...
I'm also grateful for any fellow students who gave me inspiration and some insight along the way, naturally including some additional test cases that did manage to eek  out some bugs.
Additionally, all of the program snippets I have found online around the place have been quite helpful and saved me a lot of time, including of course the source of some benchmarks.
...
As for the liveness analysis, research has been conducted allowing for liveness analysis to be conducted over IR of strict SSA form in only two passes \cite{BrandnerFlorian2011CLSf} shadowing  the current approach that depends on iterative dataflow analysis. Similarly, linear scan can be aware of 'holes' in live ranges such that variables don't need to live across widely disjointed basic blocks.
...
At first, control flow analysis was implemented to generate a control-flow graph. As this finalized,
dataflow analysis was applied to derive live ranges for each variable, which was eventually used to conduct both the graph coloring by simplifcation and linear scan algoritms. All the way throughout development a test framework has been evolving with the phases to validate correct- and soundness to some degree.
