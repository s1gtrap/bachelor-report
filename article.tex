...
The motivation for this is the fact that the \(\mathit{out}[n]\) of node \(n\) depends on \(\mathit{in}[s]\) for all successors \(s\). Since most nodes have one successor immediately after, populating this beforehands is significantly more efficient.
...
There fore it doesn't satisfy the Briggs criteria, and are not coalesced, resulting in the same interference graph as the \texttt{simple} allocator in Figure \ref{fig:countdown.dot}.
...
\section{Linear Scan}
Another approach that also depends on liveness analysis is the linear scan algorithm.
...
That said, it may generate more imprecise assignments and therefore also be less performant at runtime, so the utility of such an approach depends on context.
Some compilers typically compile code into machine code ahead of time (AOT) while others depend on just-in-time (JIT) compilation which defers the compilation of code until runtime.
Therefore linear scan is an approach to register allocation that may be used to achieve reasonably performant runtime compilation.
...
Similar to graph coloring, given \(k\) available GPRs, it assigns  as many variables as possible, and spilling the rest.

It does this by first building the intervals using the \(\mathit{in}[s]\) and \(\mathit{out}[s]\) resulting from prior dataflow analysis. This can be done in a single pass with a little creative array indexing as can be seen in Figure \ref{fig:intervals-ocaml}.

i\subsection{Heuristics spilling}
...
define i32 @main (i32 %0, i8* %1) {
  ...
  ret i32 %5
     ...
     \end{minted}
     \caption{Definition of \texttt{@main} in \texttt{tests/linear.ll}}\label{fig:linear.ll}
\end{figure}
...
\begin{tabular}{|>{\tt}l|>{\tt}l|l|} 
 ...
 \hline
add      & addx \%src, \%dst & \{\}\\
alloca   & subq\ \ \_\_\_, \%rsp & \{\texttt{\%rsp}\}\\
allocan  & subq\ \ \_\_\_, \%dst & \{\texttt{\%rsp}\}\\
and      & andx \%src, \%dst & \{\}\\
ashr     & sarx \%src, \%dst & \{\}\\
...
icmp     & cmpx \%src, \%dst & \{\texttt{\%rFLAGS}\}\\
load     & movx (\%src), \%dst & \{\}\\
lshr     & shrx \%src, \%dst & \{\}\\
...
zext     & movx \%src, \%dst & \{\}\\
 \hline
\end{tabular}
\caption{LLVM-{}- to x86 translation table}\label{tab:trans}
\end{table}
...
The \texttt{\%rax} register is zeroed before 

\subsubsection{Implementing \mintinline{llvm}{gep} instructions}\label{impl-gep}
