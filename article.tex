...
Compilers can be divided into several distinct phases, with the two halfs of which colloquially referred to as the \textit{frontend} and \textit{backend} stages. With the former serving the purpose of translating a high-level programming language to an \textit{intermediate representation} (IR) layer and the latter translating said IR to executable machine code on a target platform.
...
\begin{enumerate}
    ...
    \item  \lstinline!mov! operations \lstinline!gep!, \lstinline!phi!, \lstinline!ptrtoint! and \lstinline!zext!
...
For a given program, it's representation as linear sequence of instructions from the above set isn't immediately useful for liveness analysis. (FIXME: why not?)  hence a \textit{control flow graph} is derived from these. A terminator will branch elsewhere to continue execution. The destination of this has to be an annotated by a label, hence labels \textit{initiate} blocks.
