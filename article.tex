...
let vert g e t =
  ...
  | None ->
      ...
      (v, S.ST.add e v t)
\end{minted}
Then the interference graph is derived in Figure \ref{fig:interf}. First, all parameters are connected, then the instructions are processed as described in the Appel text \cite[222]{tiger}: phi nodes, as they are the only \texttt{mov} instructions, interference edges are added with all  \(\mathit{out}[n] \setminus \{b_1, ..., b_n\}\) (see \eqref{phi}) while all others are just \(\mathit{out}[n]\).
