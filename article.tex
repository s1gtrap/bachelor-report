...
The \texttt{dataflow} function takes a list of instructions \texttt{insns}, an array of CFG vertices \texttt{ids} as well as the CFG \texttt{g}. The \(i\)th element of \texttt{ids} is the CFG vertex corresponding to the \(i\)th entry of \texttt{insns}. The decision to use arrays instead of pairing up each instruction with both \textit{live-in} and \textit{live-out} sets was made because of the \textit{live-out} equation \eqref{flowout} that needs to find all \textit{live-out} sets of the successor blocks. Hence random access arrays were chosen instead of linear search.
...
\[
  \forall b_j \in (\mathit{out}[i] \setminus B), \mathit{add\_edge}(a, b_j)
\]
